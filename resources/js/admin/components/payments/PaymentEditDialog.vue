                ...
                <div class="relative px-8 py-8 flex items-center justify-between">
                  <div class="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <p class="text-sm text-white/80">Update the details of this financial transaction.</p>
                    </div>
                  </div>
                ...
                <form @submit.prevent="handleSubmit" class="space-y-6">
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <input v-model="form.amount" type="number" step="0.01" required class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all font-semibold text-slate-600" />
                    </div>
                    <div>
                    ...
                    <div>
                      <label class="block text-sm font-bold text-gray-700 mb-2">Transaction ID (Optional)</label>
                      <input v-model="form.transaction_id" type="text" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all font-semibold text-slate-600" />
                    </div>
                    <div class="md:col-span-2">
                      <label class="block text-sm font-bold text-gray-700 mb-2">Notes</label>
                      <textarea v-model="form.notes" rows="3" class="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-2xl focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all font-semibold resize-none text-slate-600"></textarea>
                    </div>
                  </div>
...
watch(() => props.payment, (val) => {
  if (val) {
    ...
    form.payment_date = val.payment_date ? val.payment_date.split('T')[0] : "";
...
const handleSubmit = async () => {
  loading.value = true;
  try {
    const response = await axios.put(`/api/v1/payments/${props.payment.id}`, form);
