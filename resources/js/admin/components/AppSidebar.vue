<template>
  <aside 
    :class="cn(
      'fixed inset-y-0 left-0 z-50 flex flex-col transition-all duration-300 ease-in-out border-r bg-white',
  ...
  >
    ...
    <div class="h-20 flex items-center px-6 gap-3 shrink-0 border-b border-slate-100">
      <div class="w-10 h-10 rounded-xl bg-primary flex items-center justify-center shadow-lg shadow-primary/20 shrink-0 transform transition-transform hover:scale-105 duration-300 cursor-pointer">
      ...
      <div v-if="!isCollapsed" class="flex flex-col overflow-hidden animate-in fade-in slide-in-from-left-4">
        <span class="font-bold text-base text-slate-900 tracking-tight leading-none uppercase">Benny Cards</span>
        <span class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Admin Management</span>
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar py-6 px-4 space-y-8">
      <!-- Section: General -->
      <div class="space-y-1">
        <p v-if="!isCollapsed" class="px-3 mb-3 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] leading-none">Management</p>
        <div class="space-y-1">
          <SidebarNavItem 
      ...
      <!-- Section: Reports -->
      <div class="space-y-1">
        <p v-if="!isCollapsed" class="px-3 mb-3 text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] leading-none">Analysis</p>
        <div class="space-y-1">
          <div 
            @click="toggleReports"
            :class="cn(
              'flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-all duration-200 group',
              reportsOpen && !isCollapsed ? 'bg-slate-50 text-primary font-semibold' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'
            )"
          >
            <PieChartIcon class="h-5 w-5 shrink-0 transition-colors" :class="reportsOpen && !isCollapsed ? 'text-primary' : 'text-slate-400 group-hover:text-slate-600'" />
          ...
          <!-- Report Children -->
          <div v-if="reportsOpen && !isCollapsed" class="ml-4 pl-3 border-l-2 border-slate-100 space-y-1 animate-in fade-in slide-in-from-top-2">
            <SidebarNavItem 
    ...
    <div class="p-4 border-t border-slate-100 bg-slate-50/50">
      <router-link
        v-if="!isCollapsed" 
        to="/profile"
        class="mb-4 p-3 rounded-2xl bg-white border border-slate-200 flex items-center gap-3 shadow-sm animate-in fade-in zoom-in-95 duration-500 hover:border-primary/50 transition-colors"
      >
        ...
        <div class="flex flex-col min-w-0">
          <span class="text-xs font-bold text-slate-900 truncate">{{ user?.name || 'Loading...' }}</span>
          <span class="text-[10px] text-slate-400 truncate lowercase font-medium">{{ user?.email || 'Fetching email...' }}</span>
        </div>
      </router-link>
      ...
      <div class="flex gap-2">
        <button 
          ...
          title="Sign Out"
        >
          <LogOutIcon :class="cn('h-5 w-5 transition-transform group-hover:scale-110', !isCollapsed && 'mr-2')" />
  ...
  <!-- Content Spacer -->
  <div 
    :class="cn('transition-all duration-300 shrink-0', isCollapsed ? 'w-20' : 'w-72')" 
    aria-hidden="true"
  />
...
const navMain = [
  ...
  { title: 'Settings', url: '/profile', icon: UserIcon },
];
