  ...
  >
    <component 
      ...
      :class="cn(
        ...
        isActive ? 'text-white' : 'text-slate-400 group-hover:text-slate-600 group-hover:scale-110'
      )" 
    />
    <span v-if="!isCollapsed" :class="cn('font-medium tracking-wide truncate transition-all', isActive ? 'opacity-100' : 'opacity-80 group-hover:opacity-100')">
      {{ item.title }}
    </span>
    ...
    <div 
      ...
      class="absolute -right-1 top-1/2 -translate-y-1/2 w-1 h-3 rounded-full bg-primary"
    />

    <!-- Active Indicator Pill (Expanded) -->
    <div 
      v-if="isActive && !isSubItem && !isCollapsed" 
      class="absolute right-3 w-1.5 h-1.5 rounded-full bg-white/50"
    />
  </router-link>
