            ...
            >
                    ...
                    <div>
                      ...
                      <div class="flex items-center gap-3 mt-1.5">
                        ...
                        >
                          ...
                          Created: {{ formatDate(editedRole?.created_at) }}
                        </span>
                        <span
                          class="text-xs text-slate-200 font-medium flex items-center gap-1.5 bg-white/10 px-2 py-0.5 rounded-lg border border-white/10 backdrop-blur-sm"
                        >
              ...
              >
                <div class="bg-white min-h-full p-8 rounded-b-[2rem]">
                  <div v-if="editedRole" class="max-w-4xl mx-auto space-y-8">
                    <!-- Role Name -->
                    <div
...
watch(
  ...
  (val) => {
    if (val) {
      editedRole.value = val;
      form.value.name = val.name;
      // Depending on structure, permissions might be in role.permissions already
      form.value.permission = val.permissions
        ? val.permissions.map((p) => p.name)
        : [];
    }
  },
  { immediate: true },
...
const handleSubmit = () => {
  emit("save", { ...form.value, id: editedRole.value.id });
};
